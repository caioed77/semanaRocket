...
interface FeedbackSucessStepProps {
    onFeedbackRestastRequest: () => void;
}


export function FeedbackSucessStep({
    onFeedbackRestastRequest
    ...
    return (
        <>
            ...
            <div className="flex flex-col items-center py-10 w-[304px]">
                ...
                <button
                    type="button"
                    onClick={onFeedbackRestastRequest}
                    className="py-2 px-6 mt-6 bg-zinc-800 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-700 transition-colors focus:border-brand-500 focus:ring-brand-500 focus:ring-1 focus:outline-none resize-none"
                >
                    Quero enviar outro
                </button>
            </div>
